...
def ndcg_at_k(
    ...
    actual = _discounted_cumulative_gain(
        relevance_by_id.get(item_id, 0) for item_id in ranking[:k]
    )
...
def constraint_violation_rate(
    ...
    return sum(item_id in known_violations for item_id in ranking) / len(ranking)
...
def fill_rate(ranking: Sequence[str], target_count: int) -> float:
    ...
    return min(len(ranking) / target_count, 1.0)
...
def intra_list_tag_similarity(
    ...
    similarities = (
        ...
        for left_id, right_id in combinations(ranking, 2)
    )
