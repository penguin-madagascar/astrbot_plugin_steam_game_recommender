...
无需 API Key 即可运行的游戏推荐插件。默认使用 Steam 公开数据源、规则过滤/排序和 AstrBot LLM 偏好解析；填写 RAWG API Key 后会增强多平台候选召回和 RAWG 游戏资料。安装 `astrbot_plugin_steam_price_heybox` 时，会额外补充 Steam 当前价、历史最低价、促销状态和小黑盒跨区价格摘要；未安装时仍保持推荐能力，不让 LLM 凭记忆编造事实。
