...
- `/unplayedrec`：随机选择一款未玩且评价过线的库内游戏；alias：`/未玩推荐`。
...
`/unplayedrec` 使用普通单条消息，只返回游戏名和 2–3 句玩法、口碑、知名度理由，不返回分数、价格或链接。
...
3. 直接使用 `/gamerec`。如需游戏库过滤或 `/unplayedrec`，再配置 `steam_api_key` 并绑定账号。
...
- `steam_min_review_count`：口碑贝叶斯先验强度，同时是 `/unplayedrec` 最低评测数。
- `steam_min_positive_ratio`：`/unplayedrec` 最低好评率，默认 0.65。
...
- `/unplayedrec` 将 `playtime_forever` 为 0 的条目视为未游玩。
