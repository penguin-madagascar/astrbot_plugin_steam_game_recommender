...
- 推荐流程升级为查询感知召回与混合精排两阶段架构，支持有序 Steam 标签 TF-IDF、正负参考、游戏库画像和贝叶斯平滑口碑。
- 硬条件改为“满足 / 确认违反 / 未知”三态；确认违反直接过滤，未知候选保留并限制推荐层级。
- 新增可选 Embedding Top-20 同层重排，并在 provider 不可用、超时或向量异常时自动降级。
- 最终列表增加同层 MMR 和候选回补；预算改为软排序，价格查询并发限制为 4，无预算时不改变最终顺序。
- 重新推荐支持“喜欢 / 不喜欢 / 不要第 N 款”结构化反馈，保存上一批有序摘要和最多 10 条反馈。
- 删除逐游戏 LLM 润色，推荐理由统一由 Steam 数据和结构化证据生成。
